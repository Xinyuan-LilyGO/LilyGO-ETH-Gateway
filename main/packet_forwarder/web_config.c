...
int save_config(void)
{
    ...
    for(int i = 0; i < CONFIG_NUM; i++) {
        if(config[i].val != NULL){
            ...
            printf("set %s:  val : %s res: %s\n", config[i].name,config[i].val, (err != ESP_OK) ? "Failed!" : "Done");
        }
    }
...
static int _config_wifi_mode(wifi_mode_e mode)
{
    ...
    if(mode == WIFI_MODE_ETH)
        strncpy(p, "eth_net", len+1);
    ...
    ESP_LOGE("CONFIG","CONFIG WIFI MODE : %s\n",config[WIFI_MODE].val);
...
int config_wifi_mode(wifi_mode_e mode)
{
    ...
    if(mode == WIFI_MODE_ETH)
        return _config_wifi_mode(WIFI_MODE_ETH);
